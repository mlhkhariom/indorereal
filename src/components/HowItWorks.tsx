...
const iconMap: Record<string, any> = { Search, Calendar, FileText, Home };
...
const HowItWorks = () => {
  const { data: dbSteps } = useHomepageSection("how_it_works");

  const steps = dbSteps && dbSteps.length > 0
    ? dbSteps.map((s, i) => ({
        icon: iconMap[s.icon || ""] || Search,
      ...
      <div className="container mx-auto px-4 lg:px-8 relative">
        <ScrollReveal>
          <div className="text-center max-w-2xl mx-auto">
            ...
            <p className="section-subtitle">Your property journey in four simple steps</p>
          </div>
        </ScrollReveal>
